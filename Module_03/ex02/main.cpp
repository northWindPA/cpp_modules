#include "FragTrap.hpp"
#include "ClapTrap.hpp"
#include "ScavTrap.hpp"

int main ()
{
	std::cout << "<====================================================================================================================>" << std::endl;
	FragTrap character1("FR4G-TP");
	ScavTrap character2("SCAV-C9");
	FragTrap character3("FR4G-XX");
	ClapTrap character4("CLAP-Z6");
	srand(time(0));
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.meleeAttack(character2.get_name());
	character2.takeDamage(character1.get_melee());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.rangedAttack(character2.get_name());
	character2.takeDamage(character1.get_range());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.vaulthunter_dot_exe(character2.get_name());
	character2.takeDamage(character1.get_ran_dam_tmp());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.vaulthunter_dot_exe(character2.get_name());
	character2.takeDamage(character1.get_ran_dam_tmp());
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.beRepaired(100);
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.vaulthunter_dot_exe(character2.get_name());
	character2.takeDamage(character1.get_ran_dam_tmp());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.vaulthunter_dot_exe(character2.get_name());
	character2.takeDamage(character1.get_ran_dam_tmp());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.vaulthunter_dot_exe(character2.get_name());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.vaulthunter_dot_exe(character2.get_name());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.meleeAttack(character2.get_name());
	character2.takeDamage(character1.get_melee());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.rangedAttack(character2.get_name());
	character2.takeDamage(character1.get_range());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.meleeAttack(character2.get_name());
	character2.takeDamage(character1.get_melee());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.rangedAttack(character2.get_name());
	character2.takeDamage(character1.get_range());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.meleeAttack(character2.get_name());
	character2.takeDamage(character1.get_melee());
	std::cout << "<====================================================================================================================>" << std::endl;
	character1.rangedAttack(character2.get_name());
	character2.takeDamage(character1.get_range());
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.beRepaired(200);
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character2.challengeNewcomer("Newcomer");
	std::cout << "<====================================================================================================================>" << std::endl;
	character3.vaulthunter_dot_exe(character4.get_name());
	character4.takeDamage(character3.get_ran_dam_tmp());
	std::cout << "<====================================================================================================================>" << std::endl;
	character3.vaulthunter_dot_exe(character4.get_name());
	character4.takeDamage(character3.get_ran_dam_tmp());
	std::cout << "<====================================================================================================================>" << std::endl;
	character3.vaulthunter_dot_exe(character4.get_name());
	character4.takeDamage(character3.get_ran_dam_tmp());
	std::cout << "<====================================================================================================================>" << std::endl;
	character3.vaulthunter_dot_exe(character4.get_name());
	character4.takeDamage(character3.get_ran_dam_tmp());
	std::cout << "<====================================================================================================================>" << std::endl;
	return (0);
}
